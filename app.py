...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            # exemplo: armazenar id na sessão
...
def registrar_transacao(valor, tipo, descricao, categoria):
    ...
    nova = {"Data": data, "Tipo": tipo, "Descrição": descricao, "Categoria": categoria, "Valor": valor}
...
def gerar_dashboard():
    ...
    df["Valor Ajustado"] = df.apply(lambda x: x["Valor"] if str(x.get("Tipo")).lower() == "entrada".lower() else -x["Valor"], axis=1)
    ...
    try:
        ...
        fig1.tight_layout()
        fig1.savefig("static/saldo.png", bbox_inches='tight', facecolor='white')
        plt.close(fig1)
        ...
        ax2.axis('equal')
        fig2.tight_layout()
        fig2.savefig("static/categorias.png", bbox_inches='tight', facecolor='white')
        plt.close(fig2)
...
def processar_mensagem(frase):
    ...
    valor = float(valor_match.group().replace(",", ".")) if valor_match else None
    ...
    cumprimentos = ["oi", "olá", "ei", "boa tarde", "bom dia", "boa noite"]
    ...
    return "🤔 Não entendi. Ex: 'Recebi 2000 de salário' ou 'Gastei 50 no mercado'.", None, None
